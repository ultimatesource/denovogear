...
Eigen::Matrix4d f81_matrix(double mu, std::array<double, 4> nuc_freq,
                           bool bNoMut = false) {
    double beta = 1.0;
    for(auto d : nuc_freq) {
        beta -= d * d;
...
dng::TransitionMatrix mitosis_matrix(double mu, std::array<double, 4> nuc_freq,
                                     bool bNoMut = false) {
    ...
    for(int p = 0; p < 10; ++p) {
        for(int k = 0; k < 10; ++k) {
            ret(p, k) = 0.5 * (m(nucleotides[p][0],
                                 nucleotides[k][0]) * m(nucleotides[p][1], nucleotides[k][1])
                               + m(nucleotides[p][0], nucleotides[k][1]) * m(nucleotides[p][1],
                                       nucleotides[k][0]));
            if(nucleotides[p][0] == nucleotides[p][1]) {
                continue;
            }
            ret(p, k) += 0.5 * (m(nucleotides[p][1],
                                  nucleotides[k][0]) * m(nucleotides[p][0], nucleotides[k][1])
                                + m(nucleotides[p][1], nucleotides[k][1]) * m(nucleotides[p][0],
                                        nucleotides[k][0]));
        }
    }
    return ret;
...
dng::TransitionMatrix meiosis_matrix(double mu, std::array<double, 4> nuc_freq,
                                     bool bNoMut = false) {
...
dng::GenotypeArray population_prior(double theta,
                                    std::array<double, 4> nuc_freq, std::array<double, 4> prior) {
    ...
    for(auto &row : pedigree.table()) {
        ...
        if(res == -1) {
            throw std::runtime_error(
                "unable to parse somatic data for individual '" + pedigree.name(
                    row.child) + "'.");
        } else if(res == 0) {
