...
int process_seq_data(task::Call::argument_type &arg, io::Pedigree &ped, std::array<double, 4> &freqs);
int process_variant_data(task::Call::argument_type &arg, io::Pedigree &ped, std::array<double, 4> &freqs);
...
int task::Call::operator()(Call::argument_type &arg) {
    ...
    if(mode == utility::FileCat::Sequence) {
    	// sam, bam, cram
    	return process_seq_data(arg, ped, freqs);

    }
    else if(mode == utility::FileCat::Variant) {
    	return process_variant_data(arg, ped, freqs);
    }
