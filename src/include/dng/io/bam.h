...
template<typename InFile>
BamScan<InFile>::list_type
BamScan<InFile>::operator()(location_t target_loc, pool_type &pool) {
...
public:
    ...
    BamPileup(const InFiles &range, int min_qlen = 0); : pool_{4048}, read_groups_{rg},
        min_qlen_{min_qlen};
...
template<typename InFiles>
BamPileup::BamPileup(const InFiles &range, int min_qlen = 0); : pool_{4048},
        min_qlen_{min_qlen};

template<typename InFiles, typename Func>
void BamPileup::operator()(InFiles &range, Func call_back) {
    using namespace std;
