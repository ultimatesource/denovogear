...
int processReads(char *reads_file, long dnm_pos, long hap_pos, string gt1,
                 string gt2, char variant_base) {
            ...
            if(fin2.is_open()) { // PARSE Parental GTs
                ...
                while(fin2.good()) {
                    ...
                    if((chr2 == chr1) && (dist <= window) && (hap_pos != dnm_pos)) {
                        ...
                        char temp_file[20];
                        getReadsFromBAM(bam_f, chr1, dnm_pos, hap_pos,
                                        temp_file); // get reads corresponding to the positions
