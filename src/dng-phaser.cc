...
int processReads(char *bam_f, std::string &chr1, long dnm_pos, long hap_pos, string &gt1, string &gt2, char variant_base) {
  ...
  while(sam_itr_next(fp, sam_itr, rec) >= 0) {
    ...
    std::string seq_formatted; // A string copy of the sequence, will be processed to match the cigar
	...
	break;
      default:
	std::cout << "Unable to handle cigar operation (htslib code " << op << ")." << std::endl;
	return 1;
      }
    }
...
void getReadsFromBAM(char *bam_f, string chr1, long dnm_pos, long hap_pos,  char *temp_file) {
  ...
  while(sam_itr_next(fp_b, sam_itr, rec_b) >= 0) {
    ...
    for(int a = 0; a < cigar_b_len; a++) {
      std::cout << bam_cigar_oplen(cigar_b[a]) << "[" << bam_cigar_op(cigar_b[a]) << "]";
	...
	if(fin2.is_open()) { // PARSE Parental GTs
	  ...
	  while(fin2.good()) {
	    ...
	    if((chr2 == chr1) && (dist <= window) && (hap_pos != dnm_pos)) {
	      ...
	      returnsum += processReads(bam_f, chr1, dnm_pos, hap_pos, gt1, gt2, variant_base);
	      //remove(temp_file);
	    }
