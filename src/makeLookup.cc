...
void getIndelPriors(string g_gts, int n_uniqa, int is_X) {
    ...
    if((nhit == 0) && (m_d_nuniq == 1)) {  //0 copies of ref in parents
        ...
        if(nhit == 4) {  // 4 copies of the ref, site look monomorphic in parents
            ...
            } else if((m_d_alleles[0] != m_d_alleles[1])
                      && (m_d_alleles[2] != m_d_alleles[3])) {
...
void getSNPPriors(string g_gts, int n_uniqa, int is_X) {
    ...
    for(int i = 0; i < 4; i++) { // LOOP OVER ALL 4 POSSIBLE BASES IN REFERENCE
        ...
        if(m_d_nuniq == 3) {
            g_priors[i] = 0.002 * 0.002 /
                          414;  // prior split equally amongst all triallelic cases
            continue;
        }
...
void makeIndelLookup(double PolyRate,
    ...
    lookupIndel.mrate.resize(9,
                             3); // Now Indel mrate is calculated based on length of indel dynamically
            ...
            for(int mid = 0; mid < 10; mid++) {
                ...
                if(g_n_u_alleles == 3) {
                    ...
                    if(((seq1[cid] == seq1[did] || seq1[cid] == seq2[did])
                        ...
                        if(seq1[cid] != seq1[did] && seq1[cid] != seq2[did] && seq1[cid] != seq1[mid]
                                && seq1[cid] != seq2[mid]
                                && seq2[cid] != seq1[did] && seq2[cid] != seq2[did] && seq2[cid] != seq1[mid]
                                && seq2[cid] != seq2[mid]) { // TWO MUTATIONS
                ...
                if(g_inf == 1) {  // AA/BB/AA or AA/BB/AB
                    ...
                    if((alleles[0] == 'A' || alleles[1] == 'A' || alleles[0] == 'G'
                            || alleles[1] == 'G') && (alleles[2] == 'C' || alleles[3] == 'T')) {
                        g_inf = 5;    // transversion
                    } else if((alleles[0] == 'C' || alleles[1] == 'C' || alleles[0] == 'T'
                               || alleles[1] == 'T') && (alleles[2] == 'A' || alleles[3] == 'G')) {
                        g_inf = 5;    // transversion
                    } else {
                        g_inf = 4;    // transition
                    }
                    ...
                    g_nflag = false;
                          ...
                          2) { // both alleles in mum, denovo from dad AA/AA/BB or AA/AB/BB
                    g_t_prob = 1.0;
                    g_kDenovorate = g_Mrate;
                    if((alleles[0] == 'A' || alleles[1] == 'A' || alleles[0] == 'G'
                            || alleles[1] == 'G') && (alleles[4] == 'C' || alleles[5] == 'T')) {
                        g_inf = 5;    // transversion
                    } else if((alleles[0] == 'C' || alleles[1] == 'C' || alleles[0] == 'T'
                               || alleles[1] == 'T') && (alleles[4] == 'A' || alleles[5] == 'G')) {
                        g_inf = 5;    // transversion
                    } else {
                        g_inf = 4;    // transition
                    }
...
void makePairedLookup(double pairMrate, vector<vector<string > > &tgt,
