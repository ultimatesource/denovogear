...
void usage() {
    cerr << "\nUsage:\n";
    cerr << "Autosomes:\n";
    cerr << "\tdng dnm auto --bcf bcf_f --ped ped_f [OR] dng dnm auto --vcf vcf_f --ped ped_f\n";
    cerr << "X chromosome in male offspring:\n";
    cerr << "\tdng dnm XS --bcf bcf_f --ped ped_f [OR] dng dnm XS --vcf vcf_f --ped ped_f\n";
    cerr << "X chromosome in female offspring:\n";
    cerr << "\tdng dnm XD --bcf bcf_f --ped ped_f [OR] dng dnm XD --vcf vcf_f --ped ped_f\n";
    //cerr<<"Phaser:\n";
...
int callMakeSNPLookup(vector<vector<string > > &tgtSNP, lookup_snp_t &lookupSNP,
                      string model) {
    vector<string> tmp;
    for(int l = 0; l < 10; l++) {
        tmp.push_back("NA");
...
int callMakeINDELLookup(vector<vector<string > > &tgtIndel,
                        lookup_indel_t &lookupIndel, string model) {
    vector<string> tmp;
    for(int l = 0; l < 3; l++) {
        tmp.push_back("NA");
    }
    ...
    cerr << " First code: " << lookupIndel.code(1,
            1) << " last: " << lookupIndel.code(9, 3) << endl;
    cerr << " First target string: " << tgtIndel[0][0] << " last: " <<
         tgtIndel[8][2] << endl;
    cerr << " First prior: " << lookupIndel.priors(1,
            1) << " last: " << lookupIndel.priors(9, 3) << endl;
    return 0;
}

int callMakePairedLookup(vector<vector<string > > &tgtPair,
    ...
    cerr << " First target string: " << tgtPair[0][0] << " last: " << tgtPair[9][9]
         << endl;
    cerr << " First prior " << lookupPair.priors(1,
            1) << " last: " << lookupPair.priors(10, 10) << endl;
    return 0;
}

int writeVCFHeader(std::ofstream &fo_vcf, string op_vcf_f, string bcf_file,
                   string ped_file, string sample) {
