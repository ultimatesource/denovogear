/*  Last edited: Dec 20 09:33 2008 (rd) */
/* Author: Richard Durbin, started July 2003, copyright Genome Research Limited (2003) */
...
void* remap (void *old, int oldSize, int newSize)
{
  void* new = calloc (newSize, 1) ;
...
DICT *dictCreate (int size)
{
  ...
  dict->table = (int*) calloc (dict->size, sizeof(int)) ;
  dict->names = (char**) calloc (dict->size/2, sizeof(char*)) ;
  return dict ; 
}
...
int dictFind (DICT *dict, char *s, int *ip)
{
  ...
  if (!dict) { fprintf (stderr, "dictAdd received null dict\n") ; exit (-1) ; }
  if (!s) { fprintf (stderr, "dictAdd received null string\n") ; exit (-1) ; }
...
int dictAdd (DICT *dict, char *s, int *ip)
{
  ...
  i = ++dict->max ;
  dict->table[newPos] = i ;
  dict->names[i] = (char*) malloc (strlen(s) + 1) ;
  ...
  if (dict->max > 0.3 * dict->size) /* double table size and remap */
    { int *newTable ;
      ...
      newTable = (int*) calloc (dict->size, sizeof(int)) ;
